...
/**
 * Class implementing Dijkstra's shortest path algorithm for directed simple
 * (i.e. without loops) graphs with non-negative weighted edges.
 *
...
public class Dijkstra {
    ...
    /**
     * Constructor of the Dijkstra class. This function saves the adjacency
     * matrix of a directed simple graph with weighted edges.
     *
     * The nodes are labeled by their position in the adjacency matrix. That is,
     * the vertex whose outgoing edges are saved in the ith row of adjMatrix is
     * the vertex i.
     *
     * @param adjMatrix Square matrix that represents the adjacency matrix of
     * the graph. The entry (i,j) of adjMatrix represents the weight of the edge
     * that goes from vertex i to vertex j. The weights must be positive. A
     * negative entry in the position (i,j) means that there is no edge between
     * the vertices i and j.
     *
     * @param nVertices Non-negative number of vertices of the graph, that is,
     * the order of the adjacency matrix.
     */
    ...
    /**
     * Private function that initializes all the data structures as required by
    ...
    /**
     * Returns the next vertex to be analyzed by the Dijkstra algorithm. As
     * required, that should be the unvisited vertex with smallest expected
     * cost.
     *
    ...
    /**
     * Main call of Dijkstra's shortest path algorithm. Given a directed simple
     * graph with non-negative weighted edges, an initial vertex ini and a final
     * vertex end, this function computes the shortest path between ini and end.
     *
     * The minimum distance is returned by the function. The list of nodes that
     * form the shortest path can be recovered by calling the function getPath
     * after the call to this function.
     *
     * If there is no path between ini and end, the function returns
     * Double.POSITIVE_INFINITY and no shortest path is computed.
     *
    ...
    /**
     * Get the shortest path computed by Dijkstra's algorithm. Returns an
     * ArrayList with the ordered list of nodes that form the path, from the
     * initial vertex to the final vertex.
     *
     * If there are two possible paths with minimum cost, this function returns
     * the one with less number of edges.
     *
     * If Dijkstra's algorithm has not been previously executed, the function
     * returns null and sets that an error occurred.
     *
    ...
    /**
     * Checks if an error has occurred in the execution of the class.
     *
